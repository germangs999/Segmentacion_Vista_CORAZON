...
class Network(object):        
    def __init__(self, weight_decay, learning_rate, feature_dim = 1, label_dim = 8, maxout = False):
        self.x_train = tf.compat.v1.placeholder(tf.float32, [None, 224, 224, feature_dim])
        self.y_train = tf.compat.v1.placeholder(tf.uint8, [None, label_dim])
        self.x_test = tf.compat.v1.placeholder(tf.float32, [None, 224, 224, feature_dim])
        self.y_test = tf.compat.v1.placeholder(tf.uint8, [None, label_dim])
        ...
        self.train_summary = tf.compat.v1.summary.scalar('training_accuracy', self.train_accuracy)
